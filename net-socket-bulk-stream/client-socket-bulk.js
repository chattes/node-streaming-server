...
const makeBatchedRequest = async () => {
  let watchInterval;
  let socket = net.connect({ host: "localhost", port: "3001" });
  socket.on("close", () => {
  ...
  socket.on("drain", () => {
    console.log("Okay draining buffer");
  });
  socket.on("data", (data) => {
  ...
  async function* generateData() {
    for (let i = 0; i < 10000; i++) {
      let batch = Array(400).fill(data[0]);
      let dataToWrite = JSON.stringify(batch);
